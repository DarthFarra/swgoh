...
def _get_all(ws):
    ...
    if not vals:
        return [], []
    return [h.strip() for h in vals[0]], (vals[1:] if len(vals) > 1 else [])

# ---- Los helpers siguientes son los mismos que usan tus comandos del bot ----
def map_guild_name_to_label_id_rote(ss) -> Dict[str, Tuple[str, str, str]]:
    ws = ss.worksheet(GUILDS_SHEET); headers, rows = _get_all(ws); hl = [h.lower() for h in headers]
    i_name = hl.index("guild name"); i_id = hl.index("guild id")
...
def already_synced_today(ss, guild_id: str) -> bool:
    ...
    if "guild id" not in hl or "last update" not in hl:
        return False
    i_id = hl.index("guild id"); i_last = hl.index("last update")
...
def usuarios_already_registered(ss, user_id: int, guild_name: str) -> bool:
    ws = ss.worksheet(USERS_SHEET); headers, rows = _get_all(ws); hl = [h.lower() for h in headers]
    if "user_id" not in hl or "guild_name" not in hl: return False
...
def usuarios_guilds_for_user(ss, user_id: int):
    ws = ss.worksheet(USERS_SHEET); uh, ur = _get_all(ws); ul = [h.lower() for h in uh]
    i_uid = ul.index("user_id") if "user_id" in ul else None
    i_gn  = ul.index("guild_name") if "guild_name" in ul else None
    if i_uid is None or i_gn is None: return []
    gmap = map_guild_name_to_label_id_rote(ss)
    out, seen = [], set()
    for r in ur:
        if i_uid < len(r) and str(r[i_uid]).strip() == str(user_id):
...
def upsert_usuario(ss, info: dict, tg_username: str, user_id: int, chat_id: int):
    ws = ss.worksheet(USERS_SHEET); hdr_map = ensure_usuarios_headers(ws)
    vals = ws.get_all_values() or []; rows = vals[1:] if len(vals) > 1 else []
    ...
    alias = info.get("alias",""); gname = info.get("guild_name",""); role = info.get("role",""); ally = info.get("allycode","")
    target_idx = None
    ...
    if target_idx is None:
        headers_now = ws.row_values(1) or []; new_row = [""] * len(headers_now)
        def setf(i, v): 
        ...
        row_vals = rows[target_idx][:]; need_len = len(ws.row_values(1) or [])
        if len(row_vals) < need_len: row_vals += [""] * (need_len - len(row_vals))
        def setf(i, v): 
...
def players_find_by_alias(ss, guild_name: str, alias: str):
    ws = ss.worksheet(PLAYERS_SHEET); headers, rows = _get_all(ws); hl = [h.lower() for h in headers]
    i_name = hl.index("player name"); i_gn = hl.index("guild name"); i_ac = hl.index("ally code")
    i_role = hl.index("role") if "role" in hl else (hl.index("rol") if "rol" in hl else None)
    anorm = (alias or "").strip().lower()
    for r in rows:
        gn = (r[i_gn] if i_gn < len(r) else "").strip()
        if gn != guild_name: continue
        ...
        if name.strip().lower() == anorm:
            ...
            return {"alias": name, "allycode": acd, "role": (r[i_role] if (i_role is not None and i_role < len(r)) else "").strip(), "guild_name": gn}
    return None

def players_find_by_ally(ss, guild_name: str, allycode: str):
    ws = ss.worksheet(PLAYERS_SHEET); headers, rows = _get_all(ws); hl = [h.lower() for h in headers]
    i_name = hl.index("player name"); i_gn = hl.index("guild name"); i_ac = hl.index("ally code")
    i_role = hl.index("role") if "role" in hl else (hl.index("rol") if "rol" in hl else None)
...
def user_alias_for_guild(ss, user_id: int, guild_name: str) -> Optional[str]:
    ws = ss.worksheet(USERS_SHEET); headers, rows = _get_all(ws); hl = [h.lower() for h in headers]
    i_uid = hl.index("user_id"); i_gn = hl.index("guild_name"); i_alias = hl.index("alias")
    for r in rows:
        try:
            if str(r[i_uid]).strip() != str(user_id): continue
        except Exception:
            continue
        gn = (r[i_gn] if i_gn < len(r) else "").strip()
        if gn == guild_name:
...
def render_assignments_for_alias(ss, rote_sheet: str, alias: str) -> str:
    ws = ss.worksheet(rote_sheet); headers, rows = _get_all(ws)
    if not rows: return "No hay asignaciones."
    hl = [h.lower() for h in headers]; col = {h: i for i, h in enumerate(hl)}
    for need in ("fase","planeta","operacion","personaje","reliquia","jugador"):
        if need not in col: return f"No se encontró la columna '{need}' en la hoja '{rote_sheet}'."
    i_fase, i_plan, i_op, i_char, i_rel, i_jug = (col[x] for x in ("fase","planeta","operacion","personaje","reliquia","jugador"))
    alias_norm = (alias or "").strip().lower()
    per_fase = {}
    for r in rows:
        jugador = (r[i_jug] if i_jug < len(r) else "").strip()
        if (jugador or "").strip().lower() != alias_norm: continue
    ...
    if not per_fase: return "No tienes asignaciones."
    def fase_key(fv: str):
        try: return (0, int(fv))
        except Exception: return (1, fv.lower())
    parts = []
    for fase in sorted(per_fase.keys(), key=fase_key):
        parts.append(f"*Fase {fase}*"); parts.extend(per_fase[fase]); parts.append("")
    return "\n".join(parts).strip()
