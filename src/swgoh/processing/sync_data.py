...
def fetch_localization_raw(loc_bundle: str, locale: str) -> Dict[str, Any]:
    """
    /localization con unzip + id = "{bundle}:{locale}"
    """
    return post_json(
        "/localization",
...
def parse_loc_txt_map(loc_data: Dict[str, Any], locale: str) -> Dict[str, str]:
    """
    Busca "Loc_<LOCALE>.txt" (KEY|VALUE por línea) y devuelve mapa UPPER(KEY) -> VALUE
    """
...
def run() -> Dict[str, int]:
    ...
    for s in skills:
        ...
        sid = str(s.get("id") or "")
        # Filtro por skillid usando EXCLUDE_BASEID_CONTAINS
